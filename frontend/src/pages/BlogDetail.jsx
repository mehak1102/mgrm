...
export default function BlogDetail() {
  ...
  if (!blog) {
    return (
      <main className="min-h-screen bg-[#f6f7fb] bg-app grid place-items-center">
        <div className="bg-card rounded-3xl p-10 shadow text-center">
  ...
  return (
    <main className="bg-[#f6f7fb] bg-app min-h-screen">
      <section className="max-w-5xl mx-auto px-5 py-10">
        ...
        <article className="bg-card rounded-[34px] overflow-hidden shadow-[0_30px_90px_rgba(15,23,42,0.12)] mt-8">
            ...
            <div className="grid gap-10">
              {content.map((block) => (
                <section key={block.heading}>
                  <h2 className="text-3xl font-black text-fg">{block.heading}</h2>
                  <p className="text-fg-muted leading-8 mt-3">{block.text}</p>
                </section>
              ))}
            </div>

            <div className="mt-12 rounded-[28px] bg-purple-50 bg-card border border-purple-100 border-edge p-8 transition-colors duration-300">
              <h3 className="text-3xl font-black text-fg">Quick tip</h3>
              <p className="text-fg-muted leading-8 mt-3">
